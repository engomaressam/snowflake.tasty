...
JOIN tasty_bytes_sample_data.public.franchise f
    ...
    # Query to find truck make for Sara Nicholson (using MENU_TYPE as the make)
    find_truck_query = """SELECT menu_type
...
WHERE franchisee_first_name = 'Sara' AND franchisee_last_name = 'Nicholson';"""
    ...
    if results1:
        truck_make = results1[0][0]
        print(f"\nAnswer for Question 1: truck_make = {truck_make}")
        
        # The make is in menu_type column
        if 'Chevrolet' in truck_make:
            print("✅ Correct! Answer is 'Chevrolet'")
        elif 'Volkswagen' in truck_make:
            print("✅ Correct! Answer is 'Volkswagen'")
        elif 'Airstream' in truck_make:
            print("✅ Correct! Answer is 'Airstream'")
        elif 'Nissan' in truck_make:
            print("✅ Correct! Answer is 'Nissan'")
        else:
            print(f"❌ Unexpected make: {truck_make}")
    else:
        print("No truck found for Sara Nicholson - she might not have a truck assigned")
        # Let's check if there are any trucks with the make options
        execute_query(conn, "SELECT DISTINCT menu_type FROM truck_franchise WHERE menu_type IN ('Chevrolet', 'Volkswagen', 'Airstream', 'Nissan');", "Checking available truck makes")
