...
JOIN tasty_bytes_sample_data.public.franchise f
    ...
    find_truck_query = """SELECT truck_brand_name, truck_type
FROM truck_franchise
WHERE franchisee_first_name = 'Sara' AND franchisee_last_name = 'Nicholson';"""
    
    results1 = execute_query(conn, find_truck_query, "Finding truck details for Sara Nicholson")
    
    if results1:
        truck_brand = results1[0][0]
        truck_type = results1[0][1]
        print(f"\nAnswer for Question 1: truck_brand = {truck_brand}, truck_type = {truck_type}")
        
        # The make is likely in truck_brand_name or truck_type
        # Let's check which one matches the options
        if 'Chevrolet' in truck_brand or 'Chevrolet' in truck_type:
            print("✅ Correct! Answer is 'Chevrolet'")
        elif 'Volkswagen' in truck_brand or 'Volkswagen' in truck_type:
            print("✅ Correct! Answer is 'Volkswagen'")
        elif 'Airstream' in truck_brand or 'Airstream' in truck_type:
            print("✅ Correct! Answer is 'Airstream'")
        elif 'Nissan' in truck_brand or 'Nissan' in truck_type:
            print("✅ Correct! Answer is 'Nissan'")
        else:
            print(f"❌ Unexpected make: {truck_brand} / {truck_type}")
